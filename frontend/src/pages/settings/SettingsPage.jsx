...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-heading font-bold text-gray-900">Settings & Security</h1>
        <p className="text-sm text-gray-500 font-body">Manage your account credentials, multi-factor authentication, and active sessions.</p>
      </div>

      {/* MFA Card */}
      <div className="bg-white border border-beige-200 rounded-xl shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-beige-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-saffron-50 text-saffron-600 rounded-lg">
              <Shield className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base font-heading font-semibold text-gray-900">Two-Factor Authentication (2FA)</h2>
              <p className="text-xs text-gray-500 font-body font-normal">Add an extra layer of security to your account.</p>
            </div>
          </div>
          {mfaEnabled ? (
        ...
        <div className="p-6">
          {mfaStep === 'status' && (
            <div className="space-y-4">
              ...
              {!mfaEnabled ? (
                ...
                <div className="text-sm text-gray-500 font-body flex items-center gap-2">
          ...
          {mfaStep === 'setup' && (
            <div className="space-y-6">
              <div className="space-y-2">
                <h3 className="text-sm font-semibold text-gray-800">1. Scan the QR Code</h3>
                <p className="text-xs text-gray-600">Scan this QR code with your Authenticator application, or copy the manual code below.</p>
              </div>

              {/* QR Code Layout */}
              <div className="flex flex-col items-center p-6 border border-beige-200 rounded-xl bg-beige-50 max-w-sm mx-auto">
                <div className="w-48 h-48 bg-white border border-beige-300 rounded-lg flex items-center justify-center shadow-inner relative overflow-hidden">
                  {/* Clean SVG Placeholder simulating a real QR code since we don't have a dynamic frontend QR library */}
                  <svg className="w-40 h-40 text-gray-900" viewBox="0 0 100 100">
                ...
                <div className="mt-4 w-full text-center space-y-1">
                  <p className="text-[10px] text-gray-400 uppercase font-semibold">Secret Key</p>
                  <code className="text-xs bg-white px-3 py-1.5 border border-beige-300 rounded font-mono select-all block break-all text-gray-700">
                    {mfaSecret}
                  </code>
                </div>
              </div>
          ...
          {mfaStep === 'verify' && (
            <form onSubmit={handleVerifyMfa} className="space-y-4">
              <div className="space-y-2">
                <h3 className="text-sm font-semibold text-gray-800">2. Enter Verification Code</h3>
                <p className="text-xs text-gray-600">Enter the 6-digit code displayed in your authenticator app to complete setup.</p>
              </div>
              ...
              <div className="max-w-xs">
                <input
                  ...
                  className="w-full text-center tracking-[1em] text-2xl font-mono px-4 py-3 border border-beige-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-saffron-500 focus:border-transparent text-gray-800"
                  required
                />
              </div>
      ...
      <div className="bg-white border border-beige-200 rounded-xl shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-beige-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-cardamom-50 text-cardamom-600 rounded-lg">
              <Key className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base font-heading font-semibold text-gray-900">Active Login Sessions</h2>
              <p className="text-xs text-gray-500 font-body font-normal">Audit and revoke active login sessions across all your devices.</p>
            </div>
          </div>
          {sessions.length > 1 && (
            <Button
        ...
        <div className="p-6">
          {loadingSessions ? (
            <div className="flex flex-col items-center py-10 gap-3">
              ...
              <p className="text-xs text-gray-500 font-body">Loading active sessions...</p>
            </div>
          ) : sessions.length === 0 ? (
                ...
                return (
                  <div key={s.session_id} className="py-4 first:pt-0 last:pb-0 flex items-center justify-between gap-4">
                    <div className="flex items-start gap-4 min-w-0">
                      <div className="p-2.5 bg-beige-50 border border-beige-200 text-gray-600 rounded-lg shrink-0">
                      ...
                      <div className="min-w-0 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {s.is_current && (
                            <span className="inline-flex px-2 py-0.5 text-[10px] font-bold bg-saffron-100 text-saffron-800 border border-saffron-200 rounded-full">
                              This Device
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-3 text-xs text-gray-400 font-body flex-wrap">
                          <span>IP Address: <span className="text-gray-600 font-medium">{s.ip_address}</span></span>
                          <span className="w-1.5 h-1.5 rounded-full bg-beige-300" />
                          <span>Logged in: <span className="text-gray-600 font-medium">{new Date(s.login_time).toLocaleString()}</span></span>
                        </div>
                      </div>
                    </div>
                    <Button
                      ...
                      className={s.is_current ? 'text-xs text-gray-600 border-beige-300 hover:bg-beige-50' : 'text-xs bg-red-500 hover:bg-red-600 text-white'}
                    >
