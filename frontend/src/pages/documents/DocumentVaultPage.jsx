...
export default function DocumentVaultPage() {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      {/* Header */}
      <div>
        <h1 className="font-heading font-bold text-xl text-slate-900">Document Vault</h1>
        <p className="text-sm text-slate-500 font-body">Secure access to all agricultural shipment documents.</p>
      </div>
      ...
      {loading ? (
        <div className="py-24 text-center flex flex-col items-center justify-center gap-2">
          ...
          <p className="text-sm text-slate-400 font-body">Loading document vault...</p>
        </div>
      ) : error ? (
        ...
        <div className="space-y-5">
          {filtered.map(order => (
            <div key={order.order_code} className="bg-white rounded-xl border border-agri-200 shadow-card overflow-hidden">
              <div className="px-5 py-4 border-b border-agri-100 bg-agri-50 flex justify-between items-center">
                <div>
                  <h2 className="font-heading font-semibold text-slate-900">{order.order_code}</h2>
                  <p className="text-xs text-slate-500 font-body">{order.customer_name} · {order.commodity}</p>
                </div>
                <span className="text-[11px] font-medium text-forest-700 bg-forest-50 px-2 py-1 rounded-md">
        ...
        <div className="py-16 text-center bg-white rounded-xl border border-agri-200 shadow-sm">
          <FolderOpen size={36} className="text-agri-300 mx-auto mb-3" />
          <p className="text-sm text-slate-400 font-body">No documents found matching your search.</p>
        </div>
      )}
    </div>
  )
