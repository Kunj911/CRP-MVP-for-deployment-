...
import { Search, Upload, Image, FileText, CheckCircle2 } from 'lucide-react'
import Button from '../../components/ui/Button'

const ORDERS = [
  { id:1, order_code:'ORD-2025-001', commodity_name:'Turmeric Powder',    status:'QA_TESTING'    },
  { id:2, order_code:'ORD-2025-002', commodity_name:'Cumin Seeds',        status:'DOCUMENTATION' },
  { id:3, order_code:'ORD-2025-003', commodity_name:'Black Pepper Whole', status:'PROCUREMENT'   },
]
...
export default function UploadPage() {
  ...
  const filtered = ORDERS.filter(
    (o) => o.order_code.includes(search.toUpperCase()) || o.commodity_name.toLowerCase().includes(search.toLowerCase())
  )
  ...
  function reset() {
    setStep(1); setSelectedOrder(null); setSelectedCat(null)
    setFile(null); setPreview(null); setDone(false); setSearch('')
  }
          ...
          {[1,2,3].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                step > s ? 'bg-cardamom-500 text-white' : step === s ? 'bg-saffron-500 text-white' : 'bg-beige-200 text-gray-400'
              }`}>
      ...
      {done && (
        <div className="bg-white border border-beige-200 rounded-xl p-8 flex flex-col items-center gap-3 shadow-card text-center">
          ...
          <div className="flex gap-3 w-full mt-2">
            ...
            <Button className="flex-1" onClick={() => window.location.href=`/orders/${selectedOrder?.id}`}>View Order</Button>
          </div>
        </div>
      )}
          ...
          <div className="p-4 border-b border-beige-100">
            <div className="flex items-center gap-2 bg-beige-100 rounded-lg px-3 py-2">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)}
                placeholder="Search order…" className="bg-transparent text-sm outline-none flex-1 font-body text-gray-700 placeholder-gray-400" />
            </div>
          </div>
          <div className="divide-y divide-beige-100">
            {filtered.map((o) => (
              <button key={o.id} onClick={() => { setSelectedOrder(o); setStep(2) }}
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 font-body">{o.commodity_name}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      {!done && step === 2 && (
        <div className="bg-white rounded-xl border border-beige-200 shadow-card p-4 space-y-4">
          ...
          <div className="flex bg-beige-100 rounded-lg p-1 gap-1">
            {[{k:'photo',l:'📷 Photo'},{k:'document',l:'📄 Document'}].map(({k,l}) => (
              <button key={k} onClick={() => { setUploadTab(k); setSelectedCat(null) }}
                className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${uploadTab===k?'bg-white shadow-sm text-saffron-700':'text-gray-500'}`}>{l}</button>
            ))}
          </div>
          ...
          {uploadTab === 'photo' ? (
            <div className="grid grid-cols-2 gap-2">
              {PHOTO_CATS.map((c) => (
                <button key={c.value} onClick={() => { setSelectedCat(c.value); setStep(3) }}
            ...
            <div className="space-y-2">
              {['INVOICE','BL_COPY','COA','PHYTOSANITARY_CERTIFICATE','LAB_REPORT','PACKING_LIST'].map((t) => (
                <button key={t} onClick={() => { setSelectedCat(t); setStep(3) }}
      ...
      {!done && step === 3 && (
        <div className="bg-white rounded-xl border border-beige-200 shadow-card p-4 space-y-4">
          ...
          <label className="block cursor-pointer">
            <input type="file" className="hidden" accept={uploadTab==='photo'?'image/*':'.pdf,.xlsx,.docx'} onChange={handleFile} />
            {preview ? (
              <div className="relative rounded-xl overflow-hidden aspect-video">
                <img src={preview} alt="" className="w-full h-full object-cover" />
                <div className="absolute top-2 right-2">
                  <button onClick={(e)=>{e.preventDefault();setFile(null);setPreview(null)}}
                    className="w-7 h-7 bg-white/90 rounded-full flex items-center justify-center text-gray-600 text-xs shadow">✕</button>
                </div>
              </div>
            ) : file ? (
              <div className="flex items-center gap-3 p-3 rounded-xl border border-beige-200 bg-beige-50">
                ...
                <button onClick={(e)=>{e.preventDefault();setFile(null)}} className="text-gray-400 text-xs">✕</button>
              </div>
            ) : (
              <div className="border-2 border-dashed border-beige-300 rounded-xl p-10 flex flex-col items-center gap-2 hover:border-saffron-400 hover:bg-saffron-50/30 transition-colors">
                ...
                <p className="text-[11px] text-gray-400">{uploadTab==='photo'?'JPEG, PNG, WebP, HEIC':'PDF, XLSX, DOCX'}</p>
              </div>
            )}
          </label>
